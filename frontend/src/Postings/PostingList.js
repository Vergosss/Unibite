...
//
    return (
      ...
      <tbody>
        {postings.map((posting)=><tr key={posting.id}>{details.map(detail=><td>{posting[detail]}</td>)}<td><BsFillPencilFill onClick={()=>{setAdd(true);setMode(1);setEditData(posting)}} style={{color:"gray",cursor:"pointer"}}/></td><td><BsFillTrash3Fill onClick={()=>{setAdd(1);setMode(2);setEditData(posting)}} style={{color:"red",cursor:"pointer"}}/></td></tr>)}
      </tbody>
        
        </Table>

        <Button style={{width:"100%"}} variant='primary' onClick={()=>{setAdd(true);setMode(0);setEditData({});}}>Create a new Posting</Button>
        <PostingModal show={add} mode={mode} editData={editData} onClose={()=>{setAdd(false);setEditData({})}} addPosting={addPosting} updatePosting={updatePosting} deletePosting={deletePosting} />
